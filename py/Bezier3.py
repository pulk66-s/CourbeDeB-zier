...
class Bezier3:
    ...
    def __bezier3_polynomial(self, i: int) -> Callable[float, float]:
        return lambda t: comb(3, i) * (t ** i) * (1 - t) ** (3 - i)
    ...
    def __get_equation(self, points: List[List[int]]) -> Callable[float, List[float]]:
        B0: Callable[float, float] = self.__bezier3_polynomial(0)
        B1: Callable[float, float] = self.__bezier3_polynomial(1)
        B2: Callable[float, float] = self.__bezier3_polynomial(2)
        B3: Callable[float, float] = self.__bezier3_polynomial(3)
        ...
        return lambda t: self.__add_points(
            ...
            self.__add_points(
                ...
                self.__mul_points(B3(t), points[3])
            )
        )
